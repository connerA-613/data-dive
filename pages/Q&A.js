...
function QandAPage() {
  ...
  async function fetchQuestion() {
    if (type == "STAR") {
      const completion = await complete(
        "ask me a question that would require the STAR method (situation, task, action and result) for a data analyst job interview."
      );
      setQuestion(completion);
    } else {
      const completion = await complete(
        `ask me a ${type} question for a data analyst job interview`
      );
      setQuestion(completion);
    }
  ...
  async function fetchFeedback() {
    ...
    const completion = await complete(
      `In a data analyst interview I was asked "${question}" and I answered "${input}". I need you to give me strict but consice feedback like you are the interviewer for this interview. I need you to provide feedback on the answer, explain how there answer could improve, then give a very strict rating out of 5 stars. The less correct, unprepared, or unacceptable the answer is, the worse the rating becomes. For example, if I gave a perfect answer I should recieve 5 stars, if I provide an incomplete answer or am unsure of the answer, I should get 1 star. So if I say I am not sure of the answer, I should get a failing score of 1 star.`
    );
    ...
    <>
      ...
      <Modal open={answerProvided}>
        ...
        >
          ...
          <Typography sx={{ maxHeight: "600px", overflowY: "auto" }}>
            {feedback}
          </Typography>
          {feedback ? (
            <Box
